...
public class Elevator extends JPanel implements ActionListener
{
	...
	public void setFloor(int f) {
		...
		// Problem 4:
		doorOpen = (buttons[0] && !floorBits[1] && floorBits[0]) || (buttons[1] && !floorBits[0]) || (buttons[2] && floorBits[1] && floorBits[0]); // (returns true if elevator opens)

		// Problem 5:
		direction[1]= (floorBits[1] && (buttons[0] || (buttons[2] && !floorBits[0]))) || (buttons[1] && floorBits[0]) || (buttons[2] && !floorBits[1] && !buttons[1]); //(return true if elevator moves up/down, false if stays put)
		direction[0] =(floorBits[1] && !floorBits[0] && buttons[2]) || (!floorBits[1] && floorBits[0] && buttons[2]) || (!floorBits[1] && floorBits[0] && buttons[1]); //(returns true if the elevator has to move up, returns false if it stays put/down)
		
		// Problem 6:
		lightUp = (!floorBits[1] || ((!buttons[0] || buttons[2]) && !floorBits[0])); // (True if pointing up, false is pointing down)
		lightDown = !((!floorBits[1] || ((!buttons[0] || buttons[2]) && !floorBits[0]))); // (True if pointing up, false is pointing down))
